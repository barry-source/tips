//
//  LCHasCycleIIViewController.swift
//  Leetcode
//
...
//        tail?.next = node
    ...
    /*
     快慢指针求解 val不可重复
     执行用时：64 ms, 在所有 Swift 提交中击败了81.74%的用户
     内存消耗：15 MB, 在所有 Swift 提交中击败24.77%的用户
     */
    ...
    /*
     hash存储求解 val不可重复
     执行用时：80 ms, 在所有 Swift 提交中击败了10.05%的用户
     内存消耗：15.6 MB, 在所有 Swift 提交中击败了5.05%的用户
     */
//    func detectCycle(_ head: ListNode?) -> ListNode? {
//        var head = head
//        var hashTable: [ListNode: Int] = [:]
//        while let c = head {
//            if let _ = hashTable[c] {
//                return head
//            }
//            hashTable[c] = 1
//            head = head?.next
...
//    }
    ...
    func getNode(at position: Int, head: ListNode?) -> ListNode? {
        var head = head
        var count = 0
