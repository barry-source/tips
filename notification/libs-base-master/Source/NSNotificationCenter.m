  ...
  if ([observer respondsToSelector: selector] == NO)
    ...
    */
    ...
    if (name)
    {
        ...
        */
        // m 现在是object表
        // 取不到对应的object表会创建一份新表，否则直接取出旧表
        n = GSIMapNodeForSimpleKey(m, (GSIMapKey)object);
        if (n == 0)
        {
            // 错：当object为空的时候为什么没有处理，object为空的时候，使用void 作为key
            // 当object为空的时候，GSIMapAddPair内部会将key进入转换，不会存在key为的情况
            o->next = ENDOBS;
