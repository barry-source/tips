...
- (nullable NSString *)originalCacheKeyForURL:(nullable NSURL *)url context:(nullable SDWebImageContext *)context
{
    ...
    NSString *key;
    // Cache Key Filter
    // 如果context从外部传入过滤器，直接使用，否则使用cacheKeyFilter
    id<SDWebImageCacheKeyFilter> cacheKeyFilter = self.cacheKeyFilter;
    if (context[SDWebImageContextCacheKeyFilter]) {
        cacheKeyFilter = context[SDWebImageContextCacheKeyFilter];
    }
    // 如果没有过滤器直接用absoluteString
    if (cacheKeyFilter) {
        key = [cacheKeyFilter cacheKeyForURL:url];
    }
    else {
                              ...
                              completed:(nullable SDInternalCompletionBlock)completedBlock
{
    ...
    // 查看context是否传入imageLoader，有则使用，否则使用内部的imageLoader（单例）
                                      ...
                                      completed:(nullable SDInternalCompletionBlock)completedBlock
{
    ...
    if (cacheType == SDImageCacheTypeDisk) {
        if (originalStoreCacheType == SDImageCacheTypeDisk)
            originalStoreCacheType = SDImageCacheTypeNone;
        if (originalStoreCacheType == SDImageCacheTypeAll)
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
          ...
          [self storeImage:originalImage
                ...
                imageCache:imageCache
                 cacheType:originalStoreCacheType
                  finished:finished
                completion:^{
